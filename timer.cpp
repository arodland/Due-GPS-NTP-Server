...
static void timer1_setup() {
  ...
  TC0->TC_CHANNEL[1].TC_RC = 33333333;         // Period = 1 second
...
static void timer0_setup() {
  ...
  TC0->TC_CHANNEL[0].TC_RC = 33333333;                         // Period = 1 second
  TC0->TC_CHANNEL[0].TC_RB = 33333333 - ((PPS_OFFSET_NS - PPSOUT_OFFSET_NS) / 30); // Drive high at top of second
  PIO_Configure(
    PIOB,
    PIO_PERIPH_B,
    PIO_PB27B_TIOB0,
...
static void timers_sync() {
  ...
  int32_t tgt = TC0->TC_CHANNEL[1].TC_RA + PPS_OFFSET_NS / 30 - 1;
  if (tgt < 0)
    tgt += 33333333;
  if (tgt >= 33333333)
    tgt -= 33333333;
